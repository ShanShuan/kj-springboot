...
import com.wonders.core.db.validation.constraints.BetweenDates;

@SuppressWarnings("rawtypes")
public class BetweenDatesValidator implements ConstraintValidator<BetweenDates, Object>{
	...
	public boolean isValid(Object value, ConstraintValidatorContext constraintValidatorContext) {
		...
		try {//lessDateProperty\moreDateProperty\betweenDateProperty为属性名时
			...
			}else moreDate = (Date) moreDateField.get(value);
			...
			}else betweenDate = (Date)betweenDateField.get(value); 
	...
	public static boolean validate(String begindateStr,String enddateStr,String middledateStr) throws NumberFormatException, ParseException {
		SimpleDateFormat sdf=new SimpleDateFormat("yyyy-MM-dd");
		Date begindate;
		Date enddate;
		Date middledate;
		...
		if(middledateStr!=""&&middledateStr!=null){
			...
			}else middledate=null;
		
		return validate(begindate,enddate,middledate);
	}
	public static boolean validate(Date begindate,Date enddate,Date middledate) throws NumberFormatException, ParseException {
		//middledate在begindate和enddate中间
		boolean isok=true;
		if(middledate!=null){
			if(begindate!=null){
				isok=middledate.after(begindate);
				}
			if(enddate!=null){
					isok=isok&&(middledate.before(enddate));
				}
		}else isok=false;
		return isok;
	}
	public static void main(String args[])throws ParseException, SecurityException, NoSuchFieldException, IllegalArgumentException, IllegalAccessException {
		System.out.println(BetweenDatesValidator.validate(new Date(),new Date(),new Date()));
